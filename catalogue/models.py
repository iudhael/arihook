...
class Categories(models.Model):
    ...
    nom_categorie = models.CharField('nom de la categorie', max_length=250)
...
class Modeles(models.Model): 
    ...
    nom_modele = models.CharField('nom modele', max_length=250, blank=True)
    ...
    def save(self, *args, **kwargs): # remplacer la methode de sauvegarde por ajouter des fonctionaliter au save parent
        ...
        if self.nom_modele != '':
            new_dossier = f'media/categories/{self.categorie.nom_categorie}/modele_{self.nom_modele}_{self.pk}'
        else:
        ...
        if os.path.isfile(default_chemin):
            if self.nom_modele != '':
                new_chemin_actuel = f'categories/{self.categorie.nom_categorie}/modele_{self.nom_modele}_{self.pk}/{picture_name}'
            else:
...
class Image_details(models.Model): 
    ...
    def save(self, *args, **kwargs): # remplacer la methode de sauvegarde por ajouter des fonctionaliter au save parent
        ...
        if self.img_detail_modele.nom_modele != '':
            new_dossier = f'media/categories/{self.img_detail_modele.categorie.nom_categorie}/modele_{self.img_detail_modele.nom_modele}_{self.img_detail_modele.pk}/detail_modele_{self.img_detail_modele.pk}'
            #print(new_dossier)
        else:
            new_dossier = f'media/categories/{self.img_detail_modele.categorie.nom_categorie}/modele_{self.img_detail_modele.pk}/detail_modele_{self.img_detail_modele.pk}'
        ...
        if os.path.isfile(default_chemin):
            if self.img_detail_modele.nom_modele != '':
                new_chemin_actuel = f'categories/{self.img_detail_modele.categorie.nom_categorie}/modele_{self.img_detail_modele.nom_modele}_{self.img_detail_modele.pk}/detail_modele_{self.img_detail_modele.pk}/{picture_name}'
            else:
                new_chemin_actuel = f'categories/{self.img_detail_modele.categorie.nom_categorie}/modele_{self.img_detail_modele.pk}/detail_modele_{self.img_detail_modele.pk}/{picture_name}'
